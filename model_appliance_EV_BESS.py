...
house_type = 'H2'  # Choose house_type from H1 to H9
df = pd.read_excel('H02.xlsx', sheet_name='Final', nrows=35040)  # Change excel for data.
pv_kw_cost=0.001  # In Euro. Per kW cost of PV.
wind_kw_cost=1e5  # In Euro. Per kW cost of wind installation.
E_rated=30 # BESS capacity size in kWh. BESS assumed to have 50% initial state of charge.
EV_Charger_Capacity=7   # EV charging power in kW


df_solar = pd.read_csv('resampled_pv_data - spain.csv')
...
solar_day_data = solar_data['Production in Wh'].reset_index(drop=True)
...
car_available = [value * EV_Charger_Capacity for value in car_available]
...
def objective_rule(model):
   ...
   # Objective_ev=sum(model.Pchar[ev,i]*spot_price_15min[i] for ev in range(ev_number)for i in model.T)
    return Objective_pv + Objective_wind + (Objective_spot * 365 * 20)+1e5*Objective_demand
model.Objective = Objective(rule=objective_rule, sense=minimize)

for ev in range(ev_number):   
   model.ev_energy.add(sum(model.Pchar[ev,i] for i in model.T)==energy_data_list[ev])
...
list_example = total_demand_15min_adjust.tolist()

plt.figure(figsize=(12, 6))
plt.plot(list_example, label='Original Total Demand', marker='o')
...
ax = Pchar_df.plot(y=['EV0', 'EV1'], marker='o', figsize=(10, 6))
ax.set_xlabel('Interval (15 min)')
ax.set_ylabel('Charging (kW)')
plt.show()
